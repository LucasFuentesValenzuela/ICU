...
from amod_ed.routines_icu import update_costs, update_OD, update_capacities, AoN, estimate_ri_k, update_flows, init_flows, fixed_step
...
def solve(
    ...
    try:
        while compute:
            ...
            ri_new, G_end = estimate_ri_k(G_end.copy(), ri_smoothing=ri_smoothing, a_k=1/2, ri_prev=ri_k)
...
def FW_graph_extension(G_0, OD, edge_list, ri_k, FW_tol=10**-6,
    ...
    G_k = init_flows(G_k, OD, G_prev=None)#G_prev = G_0 if you want to properly initialize
    G_k = update_costs(G_k)
